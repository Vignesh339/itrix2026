                ...
                {unlockedSnippets.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {unlockedSnippets.map((snippet) => (
                      (() => {
                        const component = components.find((c) => c.id === snippet.component_id);
                        const snippetCode =
                          component?.code_snippet && component.code_snippet.trim().length > 0
                            ? component.code_snippet
                            : `// ${snippet.component_name || "Component"} starter snippet\nvoid setup() {\n  // TODO\n}\n\nvoid loop() {\n  // TODO\n}`;

                        const detailedDescription = [
                          component?.description || "No description available.",
                          component?.category
                            ? `This component is part of the ${component.category} category and is used in the current scenario workflow.`
                            : null,
                          component?.setup_instructions
                            ? `Typical workflow: ${component.setup_instructions
                                .split("\n")
                                .map((line) => line.trim())
                                .filter(Boolean)
                                .slice(0, 6)
                          ...
                          component?.complexity_level
                            ? `Complexity level: ${component.complexity_level}.`
                            : null,
                          component?.estimated_setup_time
                            ? `Typical preparation time is around ${component.estimated_setup_time} minutes.`
                            : null,
                          component?.required_libraries && component.required_libraries.length > 0
                            ? `Commonly used libraries: ${component.required_libraries.join(", ")}.`
                            : null,
                          component?.warnings && component.warnings.length > 0
                            ? `Important notes: ${component.warnings.join(" ")}`
                            : null,
                        ]
                          .filter(Boolean)
                          .join("\n\n");
                        ...
                        return (
                          <div
                            key={snippet.id}
                            className="rounded-xl border border-cyan-200/20 bg-slate-900/55 p-4 shadow-inner shadow-cyan-950/20"
                          >
                            <div className="mb-3 flex items-center justify-between gap-3">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  {component?.category && (
                                    <p className="text-xs text-cyan-100/65">Category: {component.category}</p>
                                  )}
                                </div>
                              </div>
                            ...
                            <div className="grid gap-3 md:grid-cols-2">
                              <div className="rounded-lg border border-cyan-200/15 bg-slate-950/70 p-3 text-xs text-cyan-100/80">
                                <p className="mb-1 font-semibold text-cyan-100">Detailed Description</p>
                                <div className="max-h-44 overflow-y-auto pr-1">
                                  <p className="whitespace-pre-wrap leading-relaxed">{detailedDescription}</p>
                                </div>
                              </div>
                            </div>

                            <div className="mt-3 rounded-lg border border-cyan-200/20 bg-slate-950 p-3">
                              <p className="mb-2 text-xs font-semibold uppercase tracking-[0.12em] text-cyan-100/70">Code Snippet</p>
                              <pre className="max-h-60 overflow-auto whitespace-pre-wrap text-xs text-cyan-100">
